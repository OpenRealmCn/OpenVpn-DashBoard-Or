...
- SSH 快捷安装:在「添加子节点」弹窗填目标机 SSH 地址与凭据(密码或私钥,支持非 root + sudo),主节点远程完成面板安装、令牌生成与绑定,日志实时可见;凭据仅本次使用、绝不落盘,连接不校验主机指纹(界面有明示)
...
- 移动端自适应:手机上侧边栏收纳为抽屉导航,表格横向滚动,表单、弹窗与详情抽屉自适应小屏
...
- [ ] SSH 快捷安装:密码与私钥两种认证均能完成安装与绑定;凭据错误、目标机缺 curl、非 root 无 sudo 时错误信息可读
- [ ] 手机宽度(≤767px)访问:抽屉导航可用,各页表格可横向滚动,弹窗、表单与节点详情抽屉不溢出屏幕
